...
def  main():
  ...
  installer = "\installer\MegaSynth Installer.exe"
   
  if demo:
    installer = "\installer\MegaSynth Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\MegaSynth manual.pdf" 
  ]
  ...
  ZIPNAME = "MegaSynth-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "MegaSynth-v" + FULLVERSIONSTR + "-win-demo.zip"
