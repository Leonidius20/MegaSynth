...
if %1 == 1 (echo Making MegaSynth Windows DEMO VERSION distribution ...) else (echo Making MegaSynth Windows FULL VERSION distribution ...)
...
REM - msbuild MegaSynth-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild MegaSynth.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild MegaSynth.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MegaSynth.aaxplugin\Contents\Win32\MegaSynth.aaxplugin --out .\build-win\aax\bin\MegaSynth.aaxplugin\Contents\Win32\MegaSynth.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MegaSynth.aaxplugin\Contents\x64\MegaSynth.aaxplugin --out .\build-win\aax\bin\MegaSynth.aaxplugin\Contents\x64\MegaSynth.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\MegaSynth.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\MegaSynth.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "MegaSynth Installer" ".\installer\MegaSynth Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\MegaSynth Installer.exe" ".\installer\MegaSynth Demo Installer.exe"
REM -del ".\installer\MegaSynth Installer.exe"
REM -)
